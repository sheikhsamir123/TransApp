...
#     pickup_lat:   Optional[float] = None    # ← Optional, not just float
...
# In-memory store — fast, no DB writes needed for live location

# cd E:\TransApp\backend
# >> uvicorn main:app --reload


# ============================================================
# TRANSAPP BACKEND — LEARNING GUIDE
# FastAPI server that handles all business logic
# ============================================================

# --- IMPORTS ------------------------------------------------
# fastapi → the web framework (like a traffic controller)
# it receives requests and sends back responses
# from fastapi import FastAPI

# # CORSMiddleware → allows React (port 5173) to talk to FastAPI (port 8000)
# # without this, browser blocks the connection for security reasons
# from fastapi.middleware.cors import CORSMiddleware

# # BaseModel → lets us define the SHAPE of data we expect to receive
# # like a contract — "I expect username AND password, both strings"
# from pydantic import BaseModel

# # random → used to pick a random driver from the list
# import random


# # --- CREATE THE APP -----------------------------------------
# # This one line creates your entire FastAPI application
# # think of it as "turning on" your server
# app = FastAPI()


# # --- CORS SETUP ---------------------------------------------
# # CORS = Cross Origin Resource Sharing
# # Problem: React runs on port 5173, FastAPI runs on port 8000
# #          Browser sees them as "different origins" and BLOCKS communication
# # Solution: tell FastAPI to ALLOW requests from React's address
...
# # For now, just a Python dictionary (key = username, value = password)
# # WARNING: never store plain text passwords in real apps — use bcrypt hashing
# USERS = {
#     "rahim": "1234",
#     "karim": "5678",
# }

# # List of fake drivers
# # In real app, drivers would register themselves and be stored in PostgreSQL
# DRIVERS = [
...
# # --- DATA MODELS (SCHEMAS) ----------------------------------
# # BaseModel defines what data shape we EXPECT from React
# # If React sends wrong data, FastAPI automatically returns an error

# class LoginData(BaseModel):
#     # these are the fields we expect in the request body
...
#     username: str
#     password: str
#     confirm_password: str  # extra field for signup

# class BookRideData(BaseModel):
#     username: str
#     pickup: str        # pickup address
#     destination: str   # destination address
#     vehicle_type: str  # "Bike", "CNG", or "Car"


# # --- ENDPOINTS ----------------------------------------------
# # An endpoint = a URL that React can send requests to
# # @app.get()  → handles GET requests (fetching data)
# # @app.post() → handles POST requests (sending data)
# # async def   → runs asynchronously (doesn't block other requests)


# # GET /  → just a health check, confirms server is running
# @app.get("/")
# async def root():
#     return {"message": "TransApp server is running!"}
#     # return a dict → FastAPI automatically converts it to JSON


# # POST /login → React sends username+password, we check and respond
# @app.post("/login")
# async def login(data: LoginData):
#     # data.username and data.password come from React's request body
#     # FastAPI automatically parses the JSON and fills LoginData for us

#     # check if user exists in our fake database
#     if data.username not in USERS:
#         return {"success": False, "message": "User not found"}
#         # return early — no need to check password if user doesn't exist
...
#         # in real app: generate a real JWT token here
#         # JWT token proves identity on future requests
#     }


# # POST /signup → React sends new user data, we save and respond
# @app.post("/signup")
...
#     # save new user to our fake database
#     # in real app: hash the password first, then save to PostgreSQL
#     USERS[data.username] = data.password

#     return {
#         "success": True,
#         "message": f"Account created! Welcome {data.username}!"
#     }


# # POST /book-ride → React sends ride request, we find a driver and respond
# @app.post("/book-ride")
...
#     matching_drivers = [d for d in DRIVERS if d["vehicle"] == data.vehicle_type]

#     # pick a random driver from matching ones
#     # in real app: find nearest driver using GPS coordinates
#     driver = random.choice(matching_drivers)

#     # fake ETA (in real app: use Google Maps Directions API)
#     eta = random.randint(2, 10)

#     # fake fare calculation (in real app: use distance from Google Maps)
#     fares = {
#         "Bike": random.randint(50, 80),
#         "CNG":  random.randint(80, 120),
#         "Car":  random.randint(150, 200),
...
# #
# # HOW TO TEST:
# #   http://127.0.0.1:8000/docs  ← interactive API docs (free!)
# #
# # WHAT --reload DOES:
# #   auto restarts server when you save the file
